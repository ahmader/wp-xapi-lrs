<?php
/**
 * Section to do settings page for the wp-xapi-lrs
 */

class WP_XAPI_LRS_Admin {
	...
	public function wp_xapi_settings_init() {
		...
		if ( WP_XAPI_LRS::meets_badgeOS_dependencies() ) {
			add_settings_field(
				'wpxapi_badges',
		...
		if ( WP_XAPI_LRS::is_using_pulsepress_theme() ) {
			add_settings_field(
				'wpxapi_voting',
	...
	public function wp_xapi_options_page() {
		...
		if ( is_plugin_active_for_network( 'wp-xapi-lrs' ) ) {
	...
	public function wp_xapi_pages_render() {
		...
		<?php
		foreach ( WP_XAPI_LRS_Admin::$page_options as $key => $value ) {
	...
	public function wp_xapi_publish_render() {
		...
		<?php
		foreach ( WP_XAPI_LRS_Admin::$publish_options as $key => $value ) {
	...
	public function wp_xapi_voting_render() {
		...
		<?php
		foreach ( WP_XAPI_LRS_Admin::$pulsepress_options as $key => $value ) {
	...
	public function setup_options() {
		...
		$this->options = WP_XAPI_LRS::wpxapi_get_class_option( false );
	...
	public static function wp_xapi_sanitize_options( $input ) {
		...
		foreach ( WP_XAPI_LRS_Admin::$checkbox_options as $checkboxKeys ) {
	...
	public static function wp_xapi_check_and_run_queue() {
		...
		$finished = WP_XAPI_LRS::wpxapi_run_the_queue();
		$count = array( 'count' => 0, 'message' => 'The queue had issues and could not run.' );
		if ( $finished ) {
			$count['count'] = WP_XAPI_LRS::wpxapi_queue_is_not_empty( true );
	...
	public static function wp_xapi_page_option_to_string( $page_option_int ) {
		return WP_XAPI_LRS_Admin::$page_options[ $page_option_int ];
	}
}

$experience_api_admin = new WP_XAPI_LRS_Admin();

//==========***********----------------*************===========//
								//===== New Class ======//
////==========***********----------------*************===========//
class WP_XAPI_LRS_Network_Admin {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_run_queue', array( 'WP_XAPI_LRS_Admin', 'wp_xapi_check_and_run_queue' ) );
		add_action( 'admin_footer',  array( 'WP_XAPI_LRS_Admin', 'wp_xapi_add_js' ) );
	}
	...
	public function wp_xapi_network_settings_init() {
		...
		add_settings_field(
			'wpxapi_run_queue',
			__( 'Run xAPI Queue', 'wpxapi' ),
			array( 'WP_XAPI_LRS_Admin', 'wp_xapi_run_xapi_queue_render' ),
			'wpxapi_network',
			'wpxapi_settings_section_lrs'
		);
		add_settings_field(
	...
	*/
	...
	public function wp_xapi_network_lrs_user_setting_render() {
		...
		<?php
		foreach ( WP_XAPI_LRS_Network_Admin::$user_setting_options as $key => $value ) {
	...
	public function wp_xapi_network_lrs_admin_level_render() {
		?>
			...
			<?php
			foreach ( WP_XAPI_LRS_Network_Admin::$admin_level_options as $key => $value ) {
	...
	public static function wp_xapi_sanitize_network_options($input) {
		//fixes to make checkbox to 0 instead of default unset
		foreach ( WP_XAPI_LRS_Network_Admin::$checkbox_fields as $checkboxKeys ) {
			if ( ! isset( $input[ $checkboxKeys ] ) ) {
				$input[ $checkboxKeys ] = 0;
			}
		}
