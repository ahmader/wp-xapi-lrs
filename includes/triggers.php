 ...
 */
 ...
 WP_XAPI_LRS::register( 'page_liked', array(
 	'hooks' => array( 'warshahpress_liked_data', 'warshahpress_unliked_data' ),
 	'num_args' => array( 'warshahpress_liked_data' => 2, 'warshahpress_unliked_data' => 2 ),
 	'process' => function( $hook, $args ) {
		
		$type = isset($_REQUEST['type']) && $_REQUEST['type']==='likeThisComment' ? 'comment' : 'post';
// die($hook);
		...
		if ( $type == 'comment' ) {
			...
			$title = 'Comment: #'.$comment_id . ' | ' . get_the_title($comment->comment_post_ID);
		...
		switch ($hook) {
			case 'warshahpress_liked_data':
				$verb = array(
					'id' => 'http://activitystrea.ms/schema/1.0/like',
					'display' => array( 'en-US' => 'liked' ),
				);
				break;
			case 'warshahpress_unliked_data':
				$verb = array(
					'id' => 'http://activitystrea.ms/schema/1.0/unlike',
					'display' => array( 'en-US' => 'unliked' ),
				);
				break;
			default:
				break;
...
WP_XAPI_LRS::register( 'earned_badges', array(
	'hooks' => array( '' ),
		...
		$statement = array(
			...
			'object' => array(
				...
				'definition' => array(
					...
					'name' => array( get_locale() => get_the_title( $current_post->ID ) ),
					'description' => array( get_locale() => $current_post->post_content ),
 ...
 */
WP_XAPI_LRS::register( 'page_views', array(
	'hooks' => array( 'wp' ), //yes, kinda broad, but if singular, should be ok
	'process' => function( $hook, $args ) {
		global $post;

		if (empty($post)) $post_id =get_the_ID() ; else $post_id=$post->ID;
		...
		if (is_singular()) {
			...
			$post_title = 'Error 404 - '.get_the_title();
		} else if (is_archive()){
			$post_title = get_the_archive_title();
		} else if (is_search()){
			$post_title = get_search_query();
		} else {
			$post_title = 'n/a'.get_the_title();
		}

		if (is_singular('page')) {
			if (isset($post->post_name) && $post->post_name=='edit-your-warshah') {
				$post_id = absint($_GET['warshah_id']);
				$post_title = get_the_title( absint( $post_id ) );
				if ($_SERVER['REQUEST_METHOD']=='POST') return;
				$verb = array(
					'id' => 'http://activitystrea.ms/schema/1.0/update',
					'display' => array( 'en-US' => 'update' ),
				);
			}
			// var_dump(get_the_ID());
			// var_dump($post->post_name=='edit-your-warshah');
			// die('EDIT: '.$post_title);
			// if ($_GET['post'])
		}
		...
		$statement = array(
			'verb' => $verb,
			'object' => array(
				'id' => WP_XAPI_LRS::current_page_url(),
				'definition' => array(
					'name' => array(
						get_locale() => $post_title,
					),
					'description' => array(
						get_locale() => $description,
					),
 ...
 */
WP_XAPI_LRS::register( 'give_comments', array(
	...
	'process' => function( $hook, $args ) {
		...
		$statement = array(
			...
			'object' => array(
				...
				'definition' => array(
					'name' => array(
						get_locale() => 'Comment: #'.$comment_id . ' | ' . get_the_title( $comment->comment_post_ID ),
					),
					'description' => array(
						get_locale() => $description,
					),
 ...
 */
WP_XAPI_LRS::register( 'pulse_press_voting', array(
	...
	'process' => function( $hook, $args ) {
		...
		$object = array(
			...
			'definition' => array(
				'name' => array(
					get_locale() => get_the_title( $post_id ) . ' | ' . get_bloginfo( 'name' ),
				),
				'description' => array(
					get_locale() => $description,
				),
 ...
 */
WP_XAPI_LRS::register( 'transition_post', array(
	...
	'process' => function( $hook, $args ) {  //args in this case should be ($new_status, $old_status, $post)
		...
		$statement = array(
			...
			'object' => array(
				...
				'definition' => array(
					'name' => array(
						get_locale() => (string) $current_post->post_title,
					),
					'type' => 'http://activitystrea.ms/schema/1.0/page',
				)
			),
 ...
 */
WP_XAPI_LRS::register( 'transition_comment', array(
	'hooks' => array( 'transition_comment_status' ),
	'num_args' => array( 'transition_comment_status' => 3 ),
	'process' => function( $hook, $args ) {  //args in this case should be ($new_status, $old_status, $comment)
		global $comment;

		$current_comment = null;
		$switched_comment = false; //so we can keep track if we switched comments
		//put verb here cause we have to account for multiple possible verbs (trashed/authored for now)
		$verb = array( 'id' => 'http://activitystrea.ms/schema/1.0/author', 'display' => array( 'en-US' => 'authored' ) );

		//switch to comment passed in via args vs global one as it's old and we are updating comments
		if ( isset( $args[2] ) && ! empty( $args[2] ) && $args[2] instanceof WP_Comment ) {
			$current_comment =  $args[2];
		} else {
		...
		if ( 4 == $options['wpxapi_publish'] ) {
			if (
				( isset( $args[0] ) && 'publish' == $args[0] ) && ( isset( $args[1] ) && 'publish' != $args[1] ) && //if going from anything (excluding publish) to publish state
				( isset( $args[2] ) && $args[2] instanceof WP_Comment ) //if comment exists
			) {
				//do nothing as this should ONLY take going to published to send to xAPI statement
			} else {
				return false;
		...
		$statement = array(
			...
			'object' => array(
				'id' => get_comment_link( $current_comment->comment_ID ),
				'definition' => array(
					'name' => array(
						// 'en-US' => __('Comment', 'warshahpress') . ': ' . (string) $current_comment->post_title . ' | ' . get_bloginfo( 'name' ),
						get_locale() => 'Comment: #' . $current_comment->comment_ID . ' | ' . (string) $current_comment->post_title ,
		...
		//now get description and insert if there is something
		$description = '';
		if ( ! empty( $current_comment->comment_content ) ) {
			$description = $current_comment->comment_content;
		}
		if ( ! empty( $description ) ) {
			$statement['object']['definition']['description'] = array( get_locale() => $description );
		}
		$result = $current_comment->comment_content;
		if ( ! empty( $result ) ) {
			$statement['result_raw']['response']= $result;
		}
