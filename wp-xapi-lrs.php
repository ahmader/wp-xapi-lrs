<?php
/**
 * Plugin Name: WP xAPI LRS
 * Plugin URI: https://github/ahmader/wp-xapi-lrs
 * Description: This plugin generates and sends various xAPI statements to the specified LRS
 * Tags: xAPI
 * Author: Ahmader
 * Version: 2.0
 * Author URI: https://github/ahmader
 * License: GNU AGPLv3
...
require_once( 'wp-xapi-lrs-configs.php' );
require_once( 'includes/TinCanPHP/autoload.php' );
require_once( 'wp-xapi-lrs-queue-obj.php' );
...
class WP_XAPI_LRS {
	...
	private static function setup_lrs( $lrs ) {
		...
		//let's do it!
		switch ( $lrs ) {
			case WP_XAPI_LRS::WPXAPI_NETWORK_LRS:
				...
				break;
			case WP_XAPI_LRS::WPXAPI_SITE_LRS:
				$return_lrs = new TinCan\RemoteLRS(
	...
	public static function init() {
		...
		if ( ! WP_XAPI_LRS::check_php_version() ) {
			deactivate_plugins( plugin_basename( __FILE__ ) );
			add_action( 'admin_notices', array( 'WP_XAPI_LRS', 'php_disable_notice' ) );
		...
		if ( ( is_multisite() && is_plugin_active_for_network( 'wp-xapi-lrs/wp-xapi-lrs.php' ) ) || defined( 'WP_XAPI_MU_MODE' ) ) {
			if ( ! empty( WP_XAPI_LRS::$site_options ) || ! empty( WP_XAPI_LRS::$site_options['wpxapi_network_lrs_password'] ) && ! empty( WP_XAPI_LRS::$site_options['wpxapi_network_lrs_username'] ) && ! empty( WP_XAPI_LRS::$site_options['wpxapi_network_lrs_url'] ) ) {

				self::$lrs1 = self::setup_lrs( WP_XAPI_LRS::WPXAPI_NETWORK_LRS );
			} else {
				add_action( 'admin_notices', array( 'WP_XAPI_LRS', 'config_unset_notice' ) );
			...
			if ( ! is_multisite() &&
					 ( empty( WP_XAPI_LRS::$options['wpxapi_lrs_url'] ) ||
					   empty( WP_XAPI_LRS::$options['wpxapi_lrs_username'] ) ||
					   empty( WP_XAPI_LRS::$options['wpxapi_lrs_password'] ) ) ) {
				add_action( 'admin_notices', array( 'WP_XAPI_LRS', 'config_unset_local_notice' ) );
				error_log( 'Please tell the Site Administrator to set the username/password/URL of the LRS (for plugin: WP ExperienceA API)' );
			}
		}

		if ( 'Yes' === WP_XAPI_LRS::$site_options['wpxapi_network_lrs_stop_all'] ) {
			add_action( 'admin_notices', array( 'WP_XAPI_LRS', 'stop_all_statements_notice' ) );
			error_log( 'The Network Administrator chose to stop allowing statements to be sent to the Network level LRS.' );
		}

		if ( is_admin() || is_network_admin() ) {
			require_once( plugin_dir_path( __FILE__ ).'wp-xapi-lrs-admin.php' );
		}
	...
	public static function load() {
		...
		foreach ( WP_XAPI_LRS::$triggers as $slug => $trigger ) {
			foreach ( $trigger['hooks'] as $hook ) {
				...
				add_action(
					...
					function() use ( $trigger ) {
						WP_XAPI_LRS::send( $trigger, func_get_args() );
					},
					$priority,
					$args
				);
	...
	public static function register( $slug, $data ) {
		if ( ! WP_XAPI_LRS::$register_locked ) {
			WP_XAPI_LRS::$triggers[ $slug ] = $data;
		}
	}
	...
	public static function deregister( $slug, $data ) {
		if ( ! WP_XAPI_LRS::$register_locked ) {
			unset( WP_XAPI_LRS::$triggers[ $slug ] );
		}
	}
	...
	public static function send( $trigger, $args ) {
		...
		$actor = WP_XAPI_LRS::create_actor( $data );
		$verb = WP_XAPI_LRS::create_verb( $data );
		$object = WP_XAPI_LRS::create_object( $data );
		$context = WP_XAPI_LRS::create_context( $data );
		$result = WP_XAPI_LRS::create_result( $data );
		$attachments = WP_XAPI_LRS::create_attachments( $data );
		$timestamp = WP_XAPI_LRS::create_timestamp( $data );
		...
		WP_XAPI_LRS::post( new TinCan\Statement( apply_filters( 'wpxapi_statement', $statement ) ) );
	}
	...
	public static function post( $data ) {
		...
		if ( ( is_multisite() && is_plugin_active_for_network( 'wp-xapi-lrs/wp-xapi-lrs.php' ) ) || defined( 'WP_XAPI_MU_MODE' ) ) {
			if ( ! empty( WP_XAPI_LRS::$lrs1 ) && 'No' === WP_XAPI_LRS::$site_options['wpxapi_network_lrs_stop_network_level_only'] ) {
				$response = WP_XAPI_LRS::$lrs1->saveStatement( $data );

				if ( false === (bool) $response->success ) {
					//since it fails, we add to queue!
					WP_XAPI_LRS::wpxapi_queue_enqueue( $data, WP_XAPI_LRS::WPXAPI_NETWORK_LRS );
				}
			}
		}

		if ( ! empty( WP_XAPI_LRS::$options['wpxapi_lrs_url'] ) && ! empty( WP_XAPI_LRS::$options['wpxapi_lrs_username'] ) && ! empty( WP_XAPI_LRS::$options['wpxapi_lrs_password'] ) ) {
			$lrs2 = self::setup_lrs( WP_XAPI_LRS::WPXAPI_SITE_LRS );
			$response2 = $lrs2->saveStatement( $data );

			if ( false === (bool) $response2->success ) {
				//failed, so enqueue!
				WP_XAPI_LRS::wpxapi_queue_enqueue( $data, WP_XAPI_LRS::WPXAPI_SITE_LRS );
			}
		}
...
/**===========================================================**/
	...
	public static function create_actor( $data ) {
		...
		if ( isset( $data['actor_raw'] ) ) {
			...
			if ( false !== $user_data ) {
				...
				if ( ! empty( WP_XAPI_LRS::$site_options['wpxapi_network_lrs_user_setting'] ) && 2 == WP_XAPI_LRS::$site_options['wpxapi_network_lrs_user_setting'] ) {
					//if email
					...
					if ( empty( $unique_id ) ) {
						...
						error_log( 'Please ensure that the constants in wp-xapi-lrs-configs.php file is set properly!' );
						return;
					}
	...
	public static function meets_badgeos_dependencies() {
		$return = true;
		foreach ( WP_XAPI_LRS::$dependencies as $class => $url ) {
	...
	public static function meets_requirements() {
		$return = WP_XAPI_LRS::meets_badgeOS_dependencies();
		...
		if ( false === $return && is_admin() && current_user_can( 'activate_plugins' ) ) {
			...
			<div id='message' class='error'>
				<?php
				echo  wp_kses( WP_XAPI_LRS::dependencies_disable_notice(), array( 'div' => array( 'class' => array() ), 'strong' => array() ) );
				?>
			</div>
			<?php
	...
	public static function php_disable_notice() {
		echo "<div class='error'><strong>" . esc_html__( 'WP xAPI LRS Plugin requires PHP ' . WP_XAPI_MINIMUM_PHP_VERSION . ' or higher.', 'wpxapi' ) . '</strong></div>';
	}
	...
	public static function wpxapi_queue_is_not_emtpy_notice() {
		$count = WP_XAPI_LRS::wpxapi_queue_is_not_empty( true );
		if ( is_super_admin() && $count > 0 ) {
			$message = __( 'The WP xAPI LRS Queue is now not empty.  Current number of items: ', 'wpxapi' );
	...
	*/
	...
	private static function setup_options() {
		self::$options = WP_XAPI_LRS::wpxapi_get_class_option( false );
		self::$site_options = WP_XAPI_LRS::wpxapi_get_class_option( true );
	}
	...
	public static function wpxapi_run_the_queue() {
		if ( WP_XAPI_LRS::wpxapi_queue_is_not_empty() ) {
			...
			while ( $i < $count ) {
				...
				//get queue object
				$queue_obj = WP_XAPI_LRS::wpxapi_queue_dequeue();
				...
				if ( false === (bool) $response->success ) {
					//failed, so enqueue!
					WP_XAPI_LRS::wpxapi_queue_enqueue( $queue_obj, $queue_obj->lrs_info );
				}
			}
		}
		return true;
...
register_activation_hook( __FILE__, array( 'WP_XAPI_LRS', 'wpxapi_on_activate' ) );
...
if ( is_multisite() ) {
	...
	if ( WPMU_PLUGIN_DIR == dirname( dirname( __FILE__ ) ) ) {
		WP_XAPI_LRS::wpxapi_on_activate();
	}
}
